...
// google auth
func (s *APIServer) HandleGoogleAuth(c *gin.Context) {
	url := s.googleOAuthConfig.AuthCodeURL("state-token")
...
// google callback
func (s *APIServer) HandleGoogleCallback(c *gin.Context) {
	state := c.Query("state")
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "userinfo request failed", "details": err.Error()})
		return
	}
	...
	if resp.StatusCode != 200 {
		c.JSON(resp.StatusCode, gin.H{
			"status": "failed",
			"error":  "Google returned error status",
			"body":   string(body),
		})
		return
	}
	...
	if len(body) == 0 {
		c.JSON(http.StatusInternalServerError, gin.H{"status": "failed", "error": "Google returned empty body"})
		return
	}
	...
	// check user exists
	user, err := s.storage.GetUserByEmail(googleUser.Email)
	fmt.Println("user: ", user)
	if err != nil || user.ID == "" {
		newUser := &models.User{
			Username: &googleUser.Name,
			Email:    &googleUser.Email,
			Password: utils.StringPointer(utils.HashPassword(uuid.New().String())),
		}

		if err := s.storage.CreateAccount(newUser); err != nil {
			c.JSON(http.StatusInternalServerError, ErrorResponse{
				Status:  Failed,
